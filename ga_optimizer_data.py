...
# =========================================================
# <<< 步驟 3: 定義基因到表型的映射 (僅保留 Data) >>>
# =========================================================
# 僅優化資料集索引 s: 3 bits (b1..b3) -> {0,...,7}
S_BITS = 3 
CHROMOSOME_LENGTH = S_BITS 

# 固定架構參數 (在此設定您認為最強的基礎配置)
FIXED_DEPTH = 3
FIXED_BASE_CHANNELS = 32
FIXED_CHANNEL_MULTS = [1, 2, 4]
...
def decode_chromosome(chromosome_str):
    ...
    if len(chromosome_str) != CHROMOSOME_LENGTH:
        raise ValueError(f"染色體長度應為 {CHROMOSOME_LENGTH}")

    # 1. 解碼 Input index s (3 bits)
    s_idx = int(chromosome_str, 2)
    ...
    config = {
        ...
        "time_dim": 32,
        "depth": FIXED_DEPTH,
        "base_channels": FIXED_BASE_CHANNELS,
        "channel_mults": FIXED_CHANNEL_MULTS[:FIXED_DEPTH],
    }
    return config
